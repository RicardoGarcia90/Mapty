...
class Cycling extends Workout {
  ...
  calcSpeed() {
    //km/h
    this.speed = this.distance / (this.duration / 60); // converter min em horas.
  }
}
...
class App {
  ...
  _renderWorkoutMarker(workout) {
    L.marker(workout.coords)
      ...
      .setPopupContent(
        `${workout.type === 'running' ? '🏃‍♂️' : '🚴‍♀️'} ${workout.drescription}`
      )
      .openPopup();
  }
  ...
  _renderWorkout(workout) {
    let html = `
      <li class="workout workout--${workout.type}" data-id="${workout.id}">
        <h2 class="workout__title">${workout.drescription}</h2>
